...
2. Select a specific church (e.g., "San Sebastian Cathedral").
   - [ ] Table immediately filters to display only users assigned to that church.
3. Select "Unassigned Users".
   - [ ] Table displays only parishioners without an assigned parish.
4. Type a query in the search bar (e.g., "Father" or parishioner name).
   - [ ] Real-time filtering matches name, email, or parish name.

#### Scenario 1.3: Group by Parish Accordion Mode
1. Click the **Group by Parish** view toggle button in the top toolbar.
   - [ ] View switches from flat table to collapsible parish accordion cards.
...
2. Click on a parish accordion header (e.g., "San Sebastian Cathedral").
   - [ ] Accordion expands to reveal the full staff and parishioner roster for that church.
3. Click "Expand All" / "Collapse All" if present, or toggle individual cards.
   - [ ] Accordion opens and closes smoothly with zero layout glitch.

#### Scenario 1.4: Church Admin Scoped View
1. Log out and log in as Church Admin (`user2@gmail.com`).
2. Navigate to **Users**.
   - [ ] Church Admin only sees staff and members belonging to their assigned parish.
...
3. Scroll down to the **Parish Map Location & Coordinates** section.
   - [ ] Leaflet map container renders with OpenStreetMap tiles.
   - [ ] Default map center is City of San Jose del Monte, Bulacan (`14.8135, 121.0453`).
4. Click anywhere on the map:
   - [ ] Draggable church pin marker moves to the clicked location.
...
5. Click and drag the marker pin:
   - [ ] Pin drags smoothly and coordinates update upon release.
...
3. Click one of the search results:
   - [ ] Map smoothly pans and zooms to the selected location.
...
2. Click **Create Church**.
   - [ ] Church is created and redirects to Church Details page.
   - [ ] Church Details page displays Coordinates: `📍 Coordinates: 14.825000°, 121.060000°` and a link to `View on OpenStreetMap`.
3. Click **Edit Church**.
   - [ ] Edit form pre-loads stored latitude and longitude and renders the pin on the existing coordinates.
...
4. Pick a taken / conflicting date and time (e.g., Date: `2026-09-15`, Time: `10:00 AM`).
   - [ ] "Checking..." spinner appears momentarily beside the time field.
...
1. Inspect **Card A (Same Church)**:
   - [ ] Displays next open time slot at San Sebastian Cathedral (e.g., `11:00 AM` or next available day).
   - [ ] Displays button: **Select This Slot**.
2. Click **Select This Slot**:
   - [ ] Preferred Time input in the form instantly updates to the recommended slot.
...
2. Inspect **Card B (Nearest Alternative Parish)**:
   - [ ] Displays the closest verified diocese parish offering Baptism on that date (e.g., `Queen of Peace Parish`).
   - [ ] Displays calculated Haversine distance badge (e.g., `📍 3.2 km away`).
   - [ ] Displays button: **Select This Parish**.
3. Click **Select This Parish**:
   - [ ] URL seamlessly switches to the new parish booking route (`/churches/church-2/book`).
...
2. Navigate to an unverified parish page ([`/churches/church-unverified-1`](http://localhost:5173/churches/church-unverified-1)).
   - [ ] Header displays an amber **Verification Pending** badge.
...
2. Navigate to **Parish Applications** (`/admin/applications`) from sidebar.
   - [ ] Review queue lists pending parish applications with applicant name and status pill.
3. Click **Review & Verify** on a pending application:
   - [ ] Review modal opens with applicant info, merchant numbers, and clickable credential links (`CBCP Clergy ID / Celebret` and `Chancery Decree`).
...
2. Click the green **Start Video Call** button in the chat header:
   - [ ] **Video Conference Modal** opens with custom room name.
...
3. Test Modal Controls:
   - [ ] Click **Copy Link**: Toast / badge shows *"Copied!"* and direct meeting URL is in clipboard.
